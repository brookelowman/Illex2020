...
This version of the model includes data from the NEFSC, ME/NH, and VIMS/NEAMAP fall bottom trawl surveys and from Study Fleet haul records. The region is a user-defined extrapolation grid based on the NEFSC bottom trawl survey strata shapefile. Habitat covariates are included in the VAST model.
...
## not 100% sure if these are still necessary:
# library(knitr)
# library(rgeos)
# library(lwgeom)
...
The model outputs will be saved in a folder we create named VAST_TEST_YYYY-MM-DD, where the date is today.
```{r, create directory for model output}
today <- lubridate::today()
DateFile = paste0("../VAST_TEST_", today, "/")

dir.create(DateFile)  #only have to create this directory the first time

```

Bring in the survey data and take a quick look.
```{r, load the survey data}
NWA_illex_data1 <- read.csv("NWA_day_2020_06_25.csv") #load the survey data
tibble(NWA_illex_data1)
```

Bring in the Study Fleet data.
```{r, load and format the study fleet data}
...
nwadata <- cbind(day = as.numeric(substr(x = NWA_illex_data2$DateTimeEST, start = 9, stop = 10))
...
Combine survey and Study Fleet datasets.
```{r, combine survey and study fleet}
...
##Clean up
rm(catch, nwa_astro, nwadata)
```

Do a little bit more data formatting
```{r, formatting}
##A little more formatting on the combined data set
## IF NOT USING THE STUDY FLEET DATA, THEN RUN THE FOLLOWING:
NWA_illex_data <- NWA_illex_data1
...
## IF ONLY USING THE PRESENCE/ABSENCE COMPONENT OF THE MODEL: Follow Thorson's recommendation to jitter presence by very small amount so VAST will estimate a logistic regression model
set.seed(908)
...
n_x = 50   # Specify the number of knots (i.e. sample locations) to use in defining spatial variables. Start with 50-100 while tinkering with the model (to speed up run time) and increase knots to increase spatial resolution of the predictions.
Kmeans_Config = list("randomseed" = 1, "nstart" = 100, "iter.max" = 1e3 ) #

#model settings
# FieldConfig = c("Omega1"=1, "Epsilon1"=0, "Omega2"=0, "Epsilon2"=0) 
FieldConfig = c("Omega1"=1, "Epsilon1"=1, "Omega2"=0, "Epsilon2"=0) #number of spatial factors (Omegas) and spatio-temporal factors (Epsilons) for the probability (_1) and positive catch (_2) linear predictors ## suggest temporarily turning off spatio-temporal factors (Epsilon1 = 0 and Epsilon2 = 0) to speed up initial run 
...
Save the VAST model settings for reproducibility.
...
Make VAST extrapolation grid. See details in NWA_offshore-inshore-region.Rmd, an adaptation of code by Cecilia O'Leary, available at [https://github.com/ceciliaOLearySBU/MakeGrids/blob/master/MakeGrids].
...
Set up the dataframe of environmental variables. 
```{r, make enviro variables dataframe}

dat <- NWA_illex_data %>% dplyr::select(Lat, Lon, Year, Catch_KG, AreaSwept_km2, Vessel) %>% droplevels() #swap out Catch_KG with PresJitter to do just encounter probability
...
Create the VAST settings object based on the options specified in the first VAST chunk.
```{r, make VAST settings object}
...
# The lower and upper values are the parameter bounds. Make sure that the MLE is not hitting them. Make sure final_gradient is near zero...less than 0.0001 should be ok if everything else looks fine.
